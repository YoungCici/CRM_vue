    ...
    <div class="app-container">
        ...
        <el-form :model="form" label-width="80px" ref="form" :rules="rules">
            ...
            <el-form-item label="手机号" prop='phone'>
                <el-input name='phone' type="text" v-model="form.phone" @change="change" :disabled="editPhone"></el-input>
            </el-form-item>
            ...
            <el-form-item label="详细地址" prop='address'>
                <el-input name='address' type="textarea" v-model="form.address" @change="change"></el-input>
            </el-form-item>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        address: [{ required: true, message: '请填写收件人详细地址', trigger: 'blur' }]
      }
    }
  },
  created() {
  ...
  methods: {
    // 数据初始化
    init() {
      this.form = JSON.parse(localStorage.getItem('orderEditForm'))
      if (this.form.isEditPhone === 0) {
        // 当角色为销售经理、组长、专员时,只能编辑自己订单列表的手机号,他人的隐藏手机号不能编辑
        this.editPhone = true
      } else {
        ...
        if (valid) {
          updateEdit(this_.form).then(response => {
            if (response.code !== '200') {
              ...
              this_.$router.push({ path: '/orders/order' })
            }
          })
        } else {
          return false
